...
if [[ "${1:-}" == "--install" ]]; then
    ...
    #   11:35  — after morning session close
    #   15:05  — after afternoon session close (main update)
    CRON_LINES=$(cat <<CRON
35 11 * * 1-5 ${SCRIPT_PATH} >> ${LOG_DIR}/cron.log 2>&1 # ${CRON_TAG}
5 15 * * 1-5 ${SCRIPT_PATH} >> ${LOG_DIR}/cron.log 2>&1 # ${CRON_TAG}
CRON
)
